...
static const unsigned char encrypt_key_text[] =
    "QWl3QnNQaFR5bGZCV3AyS3FiMlNuc0VpNng3SmswUU4=";
...
void endecryptInit() {
  ...
  if (env_key && strlen(env_key) > 0) {
    INFO_MSG(
        "Using encryption key from PYRIDE_ENCRYPT_KEY environment variable.\n");
    key_source = (const unsigned char *)env_key;
  } else {
    WARNING_MSG("Using default hardcoded encryption key. Set "
                "PYRIDE_ENCRYPT_KEY environment variable for production.\n");
  }
  ...
  if (keyLen != ENCRYPTION_KEY_LENGTH) {
    if (env_key) {
      ERROR_MSG("Failed to decode custom encryption key from "
                "PYRIDE_ENCRYPT_KEY. Using fallback.\n");
    }
