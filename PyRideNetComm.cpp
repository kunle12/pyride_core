...
PyRideNetComm::PyRideNetComm(RobotDataHandler *pDataHandler)
    : pDataHandler_(pDataHandler),
      ...
      runThread_((pthread_t)NULL), activeVideoObjs_(NULL),
      ...
      clientList_(NULL), timerList_(NULL), lastTimer_(NULL), timerCount_(0),
      nextTimerID_(1), maxFD_(0), netCommEnabled_(false), keepRunning_(false) {
#ifdef WIN32
...
#ifdef WIN32
  runThread_ =
      (HANDLE)_beginthreadex(NULL, 0, &robotmodule_thread, this, 0, NULL);
  if (runThread_ == 0) {
...
bool PyRideNetComm::initUDPListener() {
  ...
  if (setsockopt(udpSocket_, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&ttl,
                 sizeof(ttl)) < 0) {
    ERROR_MSG("PyRideNetComm::initUDPListener: failed to set multicast TTL on "
              "UDP socket.\n");
#ifdef WIN32
  ...
  if (setsockopt(udpSocket_, IPPROTO_IP, IP_ADD_MEMBERSHIP,
    ...
    ERROR_MSG("PyRideNetComm::initUDPListener: failed to join multicast group "
              "on UDP socket.\n");
    close(udpSocket_);
    return false;
  }
  ...
  if (setsockopt(udpSocket_, SOL_SOCKET, SO_BROADCAST, (char *)&turnon,
                 sizeof(turnon)) < 0) {
    ERROR_MSG("PyRideNetComm::initUDPListener: failed to enable broadcast on "
              "UDP socket.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
  ...
  if (setsockopt(udpSocket_, SOL_SOCKET, SO_REUSEADDR, (char *)&turnon,
                 sizeof(turnon)) < 0) {
    ERROR_MSG("PyRideNetComm::initUDPListener: failed to enable reuse address "
              "on UDP socket.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
  ...
  if (bind(udpSocket_, (struct sockaddr *)&sAddr_, sizeof(sAddr_)) < 0) {
    ERROR_MSG("PyRideNetComm::initUDPListener: unable to bind to network "
              "interface.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
...
bool PyRideNetComm::initTCPListener() {
  ...
  if (setsockopt(tcpSocket_, SOL_SOCKET, SO_REUSEADDR, (char *)&turnon,
                 sizeof(turnon)) < 0) {
    ERROR_MSG("PyRideNetComm::initTCPListener: failed to enable reuse addr "
  ...
  if (bind(tcpSocket_, (struct sockaddr *)&sAddr_, sizeof(sAddr_)) < 0) {
    ERROR_MSG("PyRideNetComm::initTCPListener: unable to bind to network "
              "interface.\n");
#ifdef WIN32
    closesocket(tcpSocket_);
#else
  ...
  if (listen(tcpSocket_, 5) < 0) {
    ERROR_MSG("PyRideNetComm::initTCPListener: unable to listen for incoming "
              "data.\n");
#ifdef WIN32
    closesocket(tcpSocket_);
#else
...
void PyRideNetComm::processIncomingData(fd_set *readyFDSet) {
  ...
  if (netCommEnabled_) {
    if (FD_ISSET(udpSocket_, readyFDSet)) {
      int readLen =
          (int)recvfrom(udpSocket_, dgramBuffer_, PYRIDE_DEFAULT_BUFFER_SIZE, 0,
      ...
      if (readLen <= 0) {
        ...
        ERROR_MSG("PyRideNetComm::continuousProcessing: UDP packet too large "
                  "(%d bytes), max %d\n",
  ...
  while (clientPtr) {
    ...
    if (fd != INVALID_SOCKET && FD_ISSET(fd, readyFDSet)) {
      ...
      if (readLen <= 0) {
        ...
        do {
          if (*dataPtr == PYRIDE_MSG_INIT &&
              clientPtr->dataInfo.expectedDataLength == 0) { // new message
            if (readLen < 4) {
              ERROR_MSG("PyRideNetComm::continuousProcessing: "
            ...
            if (dataCount > PYRIDE_MSG_BUFFER_SIZE) { // encryption buffer size
              ...
              memcpy(clientPtr->dataInfo.bufferedData, dataPtr,
                     clientPtr->dataInfo.bufferedDataLength);
              clientPtr->dataInfo.expectedDataLength =
                  dataCount - clientPtr->dataInfo.bufferedDataLength;
              readLen = 0;
            } else if (*(dataPtr + dataCount) ==
                       PYRIDE_MSG_END) { // valid message
          ...
          } else if (clientPtr->dataInfo.expectedDataLength >
            ...
            unsigned char *cachedPtr = clientPtr->dataInfo.bufferedData +
                                       clientPtr->dataInfo.bufferedDataLength;
            if (clientPtr->dataInfo.expectedDataLength >
                (readLen - 1)) { // continue to cache
              size_t copyLen = static_cast<size_t>(readLen);
              if (clientPtr->dataInfo.bufferedDataLength + copyLen >
                  PYRIDE_MSG_BUFFER_SIZE) {
                ERROR_MSG("PyRideNetComm::continuousProcessing: "
              ...
              clientPtr->dataInfo.expectedDataLength -=
                  static_cast<int>(copyLen);
              readLen = 0;
            } else if (*(dataPtr + clientPtr->dataInfo.expectedDataLength) ==
                       PYRIDE_MSG_END) { // valid message
              memcpy(
                  cachedPtr, dataPtr,
                  static_cast<size_t>(clientPtr->dataInfo.expectedDataLength));
              this->processDataInput(
                  clientPtr, clientPtr->dataInfo.bufferedData,
                  clientPtr->dataInfo.bufferedDataLength +
                      clientPtr->dataInfo.expectedDataLength);
    ...
    if (clientPtr->fd != INVALID_SOCKET && clientPtr->nextHeartBeat > 0 &&
      ...
      if (clientPtr->missingHeartBeats > 2) {
        ERROR_MSG("Client %d has network problem. Missing heartbeat %d. Log "
                  "user off.\n",
                  clientPtr->fd, clientPtr->missingHeartBeats);
        disconnectClient(clientPtr);
      } else { // assume the heartbeat is lost and expecting next heartbeat
        clientPtr->nextHeartBeat =
            now.tv_sec + kHeartBeatWindow + 1; // allow one extra second latency
      }
    }
#endif
...
void PyRideNetComm::processUDPInput(const unsigned char *recBuffer,
                                    int recBytes, struct sockaddr_in &cAddr) {
  unsigned char *message = NULL;
  int messageSize = 0;

#ifdef USE_ENCRYPTION
  if (decryptMessage((unsigned char *)recBuffer, (int)recBytes,
                     (unsigned char **)&message, (int *)&messageSize) != 1) {
...
PyRideNetComm::ClientItem *
PyRideNetComm::createTCPTalker(struct sockaddr_in &cAddr) {
...
void PyRideNetComm::fini() {
  ...
  if (netCommEnabled_) {
#ifndef PYRIDE_REMOTE_CLIENT
    // assume all audio and video objects will be finalised by parent level
    // code.
    if (activeVideoObjs_) {
...
#ifdef USE_MULTICAST
    setsockopt(udpSocket_, IPPROTO_IP, IP_DROP_MEMBERSHIP,
               (char *)&multiCastReq_, sizeof(multiCastReq_));
#endif
...
#endif
void PyRideNetComm::processDataInput(ClientItem *client,
                                     const unsigned char *receivedMesg,
                                     const int receivedBytes) {
  unsigned char *message = NULL;
  int messageSize = 0;

#ifdef USE_ENCRYPTION
  if (decryptMessage((unsigned char *)receivedMesg, (int)receivedBytes,
                     (unsigned char **)&message, (int *)&messageSize) != 1) {
    WARNING_MSG("Unable to decrypt incoming messasge.\n");
    return;
  }
  ...
  unsigned char *commandData =
      (unsigned char *)message + PYRIDE_MSG_HEADER_SIZE;
  int commandDataLen = messageSize - PYRIDE_MSG_MIN_LENGTH;
  switch (command) {
#ifdef PYRIDE_REMOTE_CLIENT
  case ROBOT_DECLARE:
    if (client->cID) {
      WARNING_MSG("Duplicate robot declaration message. Client %d with (new) "
                  "ID %d. Ignore.\n",
                  client->cID, cID);
    } else {
      client->cID = cID;
      client->pushData =
          true; // on console, we broadcast command to every connected robot.
      /*
      char addressStr[50];
      inet_ntop( AF_INET, &client->addr.sin_addr.s_addr, addressStr,
      INET_ADDRSTRLEN ); DEBUG_MSG( "receive robot declare %s\n", addressStr );
       */
      if (pDataHandler_ && commandDataLen >= int(sizeof(RobotInfo) + 1)) {
        ...
        if (rinfo.nofaudios < 0 || rinfo.nofaudios > 8) {
          ERROR_MSG(
              "PyRideNetComm: invalid nofaudios %d in robot declaration\n",
              rinfo.nofaudios);
          break;
        }
        ...
        pDataHandler_->onRobotCreated(cID, client->addr.sin_addr.s_addr, &rinfo,
                                      &vsettings, &asettings, optionalLabels,
                                      optLabelLength);
      }
    }
    break;
  ...
  case ROBOT_DISCOVERY: {
    if (client->cID) {
      WARNING_MSG("Duplicate remote console declaration message. Console %d "
                  "with (new) ID %d. Ignore.",
                  client->cID, cID);
      break;
    }
    if (subcommand == USER_AUTH) {
      ...
      if (!pDataHandler_ ||
          !pDataHandler_->onUserLogOn(commandData, client->fd, client->addr)) {
        disconnectClient(client, false);
        break;
      }
    }
...
void PyRideNetComm::disconnectClient(ClientItem *client,
                                     bool sendNotification) {
  if (sendNotification) { // let the other end to close down the connection
                          // first
...
#ifdef USE_ENCRYPTION
    if (encryptMessage(mesg, PYRIDE_MSG_MIN_LENGTH, &outputData,
                       &outputLength) != 1) {
      return;
    }
#else
    ...
    *dataPtr = PYRIDE_MSG_END;
    ...
    if (client) {
      if (client->fd != INVALID_SOCKET) {
        ...
        client->fd = INVALID_SOCKET; // reset fd. this is also a flag for client
                                     // item removal
#ifndef PYRIDE_REMOTE_CLIENT
...
void PyRideNetComm::cleanupClient(ClientItem *client) {
  if (client && pDataHandler_) {
    if (client->dataInfo.bufferedData) {
      ...
      client->dataInfo.bufferedDataLength =
          client->dataInfo.expectedDataLength = 0;
    }
#ifdef PYRIDE_REMOTE_CLIENT
    ...
    if (client->pushImage) {
      if (client->activeVideoObj) {
        client->activeVideoObj->stop(client->addr,
                                     PYRIDE_VIDEO_STREAM_BASE_PORT);
      }
      if (client->activeAudioObj) {
        client->activeAudioObj->stop(client->addr,
                                     PYRIDE_VIDEO_STREAM_BASE_PORT + 2);
      }
...
bool PyRideNetComm::messageValidation(const unsigned char *receivedMesg,
                                      const int receivedBytes, char &cID,
...
bool PyRideNetComm::logonToRobot(const char *host,
  ...
  if (client) {
    this->clientDataSend(ROBOT_DISCOVERY, USER_AUTH, authCode,
                         SHA256_DIGEST_LENGTH, client);
    return true;
  }
  return false;
}

void PyRideNetComm::disconnectRobots() { this->disconnectClient(NULL, true); }
...
void PyRideNetComm::setImageFormat(const char cID, ImageFormat format) {
  if (cID == 0) {
    this->clientDataSend(CLIENT_COMMAND, VIDEO_FORMAT, (unsigned char *)&format,
                         1);
  } else {
    ClientItem *client = findClientFromClientList(cID);
    if (client) {
      this->clientDataSend(CLIENT_COMMAND, VIDEO_FORMAT,
                           (unsigned char *)&format, 1, client);
    }
  }
}
...
void PyRideNetComm::switchCamera(const char cID, const char vID) {
  if (cID == 0) {
    this->clientDataSend(CLIENT_COMMAND, VIDEO_SWITCH, (unsigned char *)&vID,
                         1);
  } else {
    ClientItem *client = findClientFromClientList(cID);
    if (client) {
...
void PyRideNetComm::issueExtendedCommand(const char cID,
                                         const PyRideExtendedCommand command,
  ...
  if (cID == 0) {
    this->clientDataSend(CLIENT_COMMAND, CUSTOM_COMMAND, data,
                         optionalDataLength + 1);
  } else {
    ClientItem *client = findClientFromClientList(cID);
    if (client) {
      this->clientDataSend(CLIENT_COMMAND, CUSTOM_COMMAND, data,
                           optionalDataLength + 1, client);
    }
  }
  delete[] data;
}

void PyRideNetComm::processRobotResponse(ClientItem *client, int subcommand,
  ...
  case VIDEO_FORMAT:
    if (pDataHandler_) {
      pDataHandler_->onImageFormatChange(client->cID,
                                         (ImageFormat)(*commandData));
    }
    break;
  case VIDEO_SWITCH:
    if (pDataHandler_) {
      pDataHandler_->onVideoSwitchChange(client->cID,
                                         (VideoSettings *)(commandData));
    }
    break;
  case CUSTOM_COMMAND:
    if (dataLen >= 1 && pDataHandler_) {
      ...
      pDataHandler_->onExtendedCommandResponse(client->cID, command, optionData,
                                               dataLen - 1);
    }
    break;
  default:
...
void PyRideNetComm::processTelemetryData(ClientItem *client,
  ...
  if (objDataLen < 0) {
    ...
    if (objDataLen % sizeof(FieldObject) ==
        0) { // we have observed field objects
      pDataHandler_->onRobotTelemetryData(
          client->cID, (RobotPose *)data,
          (FieldObject *)(data + sizeof(RobotPose)),
...
void PyRideNetComm::processOperationalData(ClientItem *client,
                                           const unsigned char *data,
                                           int dataLen) {
  if (!pDataHandler_) {
    return;
  ...
  pDataHandler_->onOperationalData(client->cID, (int)status, optionalData,
                                   dataLen - 1);
}
...
void PyRideNetComm::declareRobot(const RobotInfo *robotInfo,
  ...
  rinfo.capabilities = robotInfo->capabilities;
  ...
  // Combine pDataHandler and the RobotPose data to be sent, modify the length
  // of the data as well in the argument
  memcpy(dataPtr, (void *)&rinfo, sizeof(RobotInfo));
  dataPtr += sizeof(RobotInfo);
  ...
  if (rinfo.nofcams > 0) {
    ...
    for (size_t i = 0; i < activeVideoObjs_->size(); i++) {
      ...
      *dataPtr++ = (char)cam->deviceLabel().length();
      memcpy(dataPtr, cam->deviceLabel().c_str(), cam->deviceLabel().length());
...
void PyRideNetComm::processConsoleCommand(ClientItem *client, int subcommand,
                                          const unsigned char *commandData,
                                          int dataLen) {
  ...
  case VIDEO_START:
    if (client->activeVideoObj) {
      if (client->activeVideoObj->start(client->addr,
                                        PYRIDE_VIDEO_STREAM_BASE_PORT)) {
        client->pushImage = true;
        if (client->activeAudioObj) {
          client->activeAudioObj->start(client->addr,
                                        PYRIDE_VIDEO_STREAM_BASE_PORT + 2);
        }
      }
    }
    ...
    if (dataLen == 1 && activeVideoObjs_) {
      ...
      if (client->activeVideoObj != activeVideoObjs_->at(vid)) {
        ...
        if (oldVObj->stop(client->addr, PYRIDE_VIDEO_STREAM_BASE_PORT)) {
          // previous video were streaming, so keep this way on the new video
          // object
          client->activeVideoObj->start(client->addr,
                                        PYRIDE_VIDEO_STREAM_BASE_PORT);
        }
        VideoSettings vsettings;
  ...
  case CUSTOM_COMMAND:
    if (dataLen >= 1 && pDataHandler_) {
      ...
      unsigned char
          retData[2 + sizeof(int)]; // cmd + success + return value (int)
      ...
      *retval = 0;
      if (cmd == EXCLUSIVE_CTRL_REQUEST) {
        ...
        } else if (pDataHandler_ &&
                   (*retval = pDataHandler_->onExclusiveCtrlRequest(
                        client->fd)) != 0) {
          retData[1] = EXCLUSIVE_CTRL_REJECT;
        } else {
          exclusiveCtrlClient_ = client;
          retData[1] = 1;
          clientDataSend(CLIENT_RESPONSE, subcommand, (unsigned char *)&retData,
                         2 + sizeof(int), client);
        ...
        if (exclusiveCtrlClient_ == client) {
          ...
          this->clientDataSend(ROBOT_STATUS, 0, &data, 1, NULL, false, true);
          if (pDataHandler_) {
            pDataHandler_->onExclusiveCtrlRelease(client->fd);
          }
          return;
        } else {
          retData[1] = EXCLUSIVE_CTRL_REJECT;
        ...
        retData[1] = (unsigned char)pDataHandler_->executeRemoteCommand(
            commandData, dataLen, *retval);
      } else {
        retData[1] = EXCLUSIVE_CTRL_REJECT;
      }
      clientDataSend(CLIENT_RESPONSE, subcommand, (unsigned char *)&retData,
  ...
  clientDataSend(CLIENT_RESPONSE, subcommand, (unsigned char *)&retData, 1,
                 client);
}

void PyRideNetComm::dispatchTelemetryData(const unsigned char *data,
                                          const int size) {
  this->clientDataSend(ROBOT_TELEMETRY, 0, data, size);
}

void PyRideNetComm::dispatchStatusData(const unsigned char *data,
                                       const int size) {
  this->clientDataSend(ROBOT_STATUS, 0, data, size);
}

void PyRideNetComm::takeCameraSnapshot(
    const VideoDeviceDataHandler *dataHandler, bool allCameras) {
  if (!activeVideoObjs_) {
    return;
  }
...
void PyRideNetComm::clientDataSend(const int command, const int subcommand,
  ...
  *msgPtr = PYRIDE_MSG_END;
...
#ifdef USE_ENCRYPTION
  if (encryptMessage(replyMesg, replyLength, (unsigned char **)&outputData,
                     &outputLength) != 1) {
    delete[] replyMesg;
    return;
  }
  ...
  if (useUDP) {
    sendto(udpSocket_, outputData, outputLength, 0, (struct sockaddr *)&bcAddr_,
           sizeof(bcAddr_));
  } else {
...
PyRideNetComm::ClientItem *
PyRideNetComm::findClientFromClientList(struct sockaddr_in &cAddr) {
#ifdef WIN32
  EnterCriticalSection(&t_criticalSection_);
#else
  pthread_mutex_lock(&t_mutex_);
...
PyRideNetComm::ClientItem *
PyRideNetComm::findClientFromClientList(SOCKET_T fd) {
#ifdef WIN32
  EnterCriticalSection(&t_criticalSection_);
#else
  pthread_mutex_lock(&t_mutex_);
...
PyRideNetComm::ClientItem *
PyRideNetComm::findClientFromClientList(const char cID) {
#ifdef WIN32
  EnterCriticalSection(&t_criticalSection_);
#else
  pthread_mutex_lock(&t_mutex_);
...
bool PyRideNetComm::findClientAddress(const char cID,
                                      struct sockaddr_in &cAddr) {
...
PyRideNetComm::ClientItem *
PyRideNetComm::addFdToClientList(const SOCKET_T &fd,
                                 struct sockaddr_in &cAddr) {
  ...
  /*
  int turnon = 1;
#ifndef PYRIDE_REMOTE_CLIENT // this option seem to cause issue on PR2 return
status message if (setsockopt( fd, IPPROTO_TCP, TCP_NODELAY, (char *) &turnon,
sizeof(int)) < 0) { ERROR_MSG( "Unable to set TCP_NODELAY to socket %d\n", fd );
  }
#endif

  turnon = kTCPMSS;
  if (setsockopt( fd, IPPROTO_TCP, TCP_MAXSEG, (char *) &turnon, sizeof(int)) <
0) { ERROR_MSG( "Unable to set TCP MSS to socket %d\n", fd );
  }
  */
  return newClient;
}
...
bool PyRideNetComm::getMyIPAddress(int &addr) {
  ...
  if (tcpSocket_ == INVALID_SOCKET) {
    ...
    int rc = WSAIoctl(tcpSocket_, SIO_ADDRESS_LIST_QUERY, NULL, 0, NULL, 0,
    ...
    if (rc == SOCKET_ERROR &&
      ...
      if (buf != NULL) {
        rc = WSAIoctl(tcpSocket_, SIO_ADDRESS_LIST_QUERY, NULL, 0, buf,
                      dwBytesRet, &dwBytesRet, NULL, NULL);
        if (rc != SOCKET_ERROR) {
...
#else
      pthread_cancel(tmpPtr->timerThread); // TODO should install proper
                                           // cancellation cleanup routine.
#endif
    }
    delete tmpPtr;
  }
...
void PyRideNetComm::checkTimers() {
  ...
  while (timerPtr) {
    ...
    if (timerPtr->nextTrigTime <= nowin10th && timerPtr->remainCount != 0) {
      ...
      if (timerPtr->isExecuting) {
        ERROR_MSG(
            "Timer %u is still executing while it is called again! Skip\n",
            (int)timerPtr->tID);
      } else {
        ...
        ted->mainObj = this;
#ifdef WIN32
        timerPtr->timerThread = (HANDLE)_beginthreadex(
            NULL, 0, &robottimer_thread, (void *)ted, 0, NULL);
        if (timerPtr->timerThread == 0) {
          ERROR_MSG("Unable to create a timer thread.\n");
          delete ted;
        }
#else
        if (pthread_create(&(timerPtr->timerThread), NULL, robottimer_thread,
                           (void *)ted)) {
          ERROR_MSG("Unable to create a timer thread.\n");
          delete ted;
