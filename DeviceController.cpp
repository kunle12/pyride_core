...
void DeviceController::dispatchData(const unsigned char *data,
  ...
  const AppDataUnit *adu =
      streamSession_->getData(streamSession_->getFirstTimestamp());
  if (adu) {
    delete adu;
  }
}
...
bool DeviceController::getUDPSourcePorts(short &dataport, short &ctrlport) {
  ...
  if (!streamSession_->addDestination(InetHostAddress("127.0.0.1"),
                                      PYRIDE_CONTROL_PORT)) {
  ...
  while (!found) {
    ...
    if (FD_ISSET(controlSocket, &readyFDSet)) {
      (void)recvfrom(controlSocket, dataBuffer, 200, 0, (sockaddr *)&xAddr,
                     (socklen_t *)&xLen);
      dataport = xAddr.sin_port;
    }
    if (FD_ISSET(dataSocket, &readyFDSet)) {
      (void)recvfrom(dataSocket, dataBuffer, 20, 0, (sockaddr *)&xAddr,
                     (socklen_t *)&xLen);
...
bool VideoDevice::YUV422ToRGB24(unsigned char *dstData, int dstDataSize,
                                const unsigned char *srcData,
                                const int srcDataSize) {
  ...
  while (dataPtr < dend) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      *dataPtr++ = clamp((int)*srcDataPtr - 0.396 * ((int)u - 128) -
                         0.581 * ((int)v - 128));
...
bool VideoDevice::YUV422To24(unsigned char *dstData, int dstDataSize,
                             const unsigned char *srcData,
                             const int srcDataSize) {
  unsigned char *dataPtr = dstData;
  unsigned char *srcDataPtr = (unsigned char *)srcData;
...
bool VideoDevice::RGBAToRGB24(unsigned char *dstData, int dstDataSize,
                              const unsigned char *srcData,
                              const int srcDataSize) {
  unsigned char *dataPtr = dstData;
  unsigned char *srcDataPtr = (unsigned char *)srcData;
...
bool VideoDevice::BGRAToRGB24(unsigned char *dstData, int dstDataSize,
                              const unsigned char *srcData,
                              const int srcDataSize) {
  unsigned char *dataPtr = dstData;
  unsigned char *srcDataPtr = (unsigned char *)srcData;
...
VideoDevice::VideoDevice()
    : dataHandler_(NULL), outBuffer_(NULL), outBufferSize_(0),
#ifndef JPEG62
      jpegMemBuffer_(NULL), jpegMemBufferSize_(0),
#endif
      imageWidth_(320), imageHeight_(240), imageSize_(320 * 240) {
  vSettings_.fps = 5;
...
void VideoDevice::setProcessParameters() {
  ...
  jpeg_set_quality(
      &cinfo_, kCompressionRate[static_cast<int>(vSettings_.resolution)], TRUE);
}
...
bool VideoDevice::start(struct sockaddr_in &cAddr, unsigned short cDataPort) {
  ...
  if (streamSession_->addDestination(InetHostAddress(cAddr.sin_addr),
                                     cDataPort)) {
    clientNo_++;
    retVal = true;
  }
...
bool VideoDevice::stop(struct sockaddr_in &cAddr, unsigned short cDataPort) {
  ...
  if (streamSession_->forgetDestination(InetHostAddress(cAddr.sin_addr),
                                        cDataPort)) {
    clientNo_--;
    retVal = true;
  }
...
bool VideoDevice::processImageData(const unsigned char *rawData,
                                   const int rawDataSize,
...
int VideoDevice::compressToJPEG(const unsigned char *imageData,
...
void VideoDevice::processAndSendImageData(const unsigned char *imageData,
                                          const int imageDataSize,
                                          ImageFormat format) {
  int pDataSize = 0;
  unsigned char *pData = NULL;

  if (processImageData(imageData, imageDataSize, pData, pDataSize, format,
                       true)) {
...
void VideoDevice::saveToJPEG(const unsigned char *imageData,
                             const int imageDataSize, ImageFormat format) {
  int pDataSize = 0;
  unsigned char *pData = NULL;
...
int VideoDevice::compressToHalf(const unsigned char *imageData,
                                const int imageDataSize,
                                unsigned char *&compressedData) {
...
AudioDevice::AudioDevice() : audioEncoder_(NULL), encodedAudio_(NULL) {
...
bool AudioDevice::setProcessParameters() {
  ...
  if (supported && aSettings_.channels < 3) {
    int err = 0;
    audioEncoder_ = opus_encoder_create(
        aSettings_.sampling, aSettings_.channels, OPUS_APPLICATION_VOIP, &err);
    ...
    if (audioEncoder_ && err == OPUS_OK) {
      ...
      maxEncodedDataSize_ =
          int(1 / PYRIDE_AUDIO_FRAME_PERIOD) * 60 + aSettings_.sampling;
  ...
  ERROR_MSG("Unable to initialise Opus audio encoder, check sampling rate and "
            "channels.\n");
  return false;
}
...
void AudioDevice::processAndSendAudioData(const signed short *data,
                                          const int nofSamples) {
  if (!audioEncoder_)
    return;
