...
PyModuleExtension::PyModuleExtension(const char *name)
    : clientID_(0), pow_(NULL), pPyModule_(NULL),
      pyModuleCommandHandler_(NULL) {
  name_ = name;
}

PyModuleExtension::~PyModuleExtension() { this->fini(); }
...
void PyModuleExtension::fini() {
  if (pPyModule_) {
    ...
    ServerDataProcessor::instance()->removeCommandHandler(
        pyModuleCommandHandler_);
...
bool PyModuleExtension::invokeCallback(const char *fnName, PyObject *arg) {
  ...
  PyObject *callbackFn =
      PyObject_GetAttrString(pPyModule_, const_cast<char *>(fnName));
  if (!callbackFn || callbackFn == Py_None) {
    ...
    retval = true; // as long as there is a function link to the callback, we
                   // consider it is rightly consumed.
...
void PyModuleExtension::swapCallbackHandler(PyObject *&master,
                                            PyObject *newObj) {
  if (newObj) {
    if (master) {
      Py_DECREF(master);
...
PyModuleExtendedCommandHandler::PyModuleExtendedCommandHandler(
    PyModuleExtension *pyExtModule) {
 ...
 *  \brief Callback function when a custom command is received from a remote
 * client. \param int cmd_id. Custom command ID. \param str cmd_text. Custom
 * command text string. \return None. \note The remote client must take the
 * exclusive control of the robot before its commands can trigger this callback
 * function.
 */
bool PyModuleExtendedCommandHandler::executeRemoteCommand(
    PyRideExtendedCommand command, int &retVal,
  ...
  if (result) {
    if (PyBool_Check(result)) {
      ...
      PyErr_Format(PyExc_TypeError, "onRemoteCommand callback return value "
                                    "should be an integer or boolean");
    }
  }
  Py_DECREF(arg);
  Py_XDECREF(result);
 ...
 */
int PyModuleExtendedCommandHandler::onExclusiveCtrlRequest(
 ...
 */
/**@}*/
void PyModuleExtendedCommandHandler::onExclusiveCtrlRelease(
    const std::string &username) {
  if (!pyExtModule_)
    return;
 ...
 *  \memberof ROBOT_MODEL_DOXYGEN.
 *  \brief Callback function when a timer object (created by
 * ROBOT_MODEL_DOXYGEN.addTimer) is fired. \param int timer_id. ID of the timer
 * object. \return None.
 */
 ...
 *  \memberof ROBOT_MODEL_DOXYGEN.
 *  \brief Callback function when a timer object (created by
 * ROBOT_MODEL_DOXYGEN.addTimer) is fired for the last time. \param int
 * timer_id. ID of the timer object. \return None. \note This callback function
 * works only on timers with limited life span.
 */
/**@}*/
 ...
 *  \memberof ROBOT_MODEL_DOXYGEN.
 *  \brief Callback function when a team message is received from a team member
 * (calling ROBOT_MODEL_DOXYGEN.sendTeamMessage). \param int sender_id. The ID
 * of the team member who sends the message. \param str message. Text of the
 * message. \return None.
 */
/**@}*/
/**@{*/
 ...
 *  \memberof ROBOT_MODEL_DOXYGEN.
 *  \brief Callback function when ROBOT_MODEL_DOXYGEN.takeCameraSnapshot is
 * called. \param str image_name. Path for the saved image. \return None.
 */
/**@}*/
