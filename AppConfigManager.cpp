...
void AppConfigManager::loadDeviceInfo(TiXmlNode *deviceInfoNode) {
  ...
  if (loadErrors > 0) {
    WARNING_MSG(
        "LoadConfig: configration file contains %d bad video device record.",
        loadErrors);
  }
}
...
bool AppConfigManager::signInUserWithPassword(const unsigned char *code,
                                              SOCKET_T fd,
...
bool AppConfigManager::addUser(const char *name, const char *password) {
  ...
  for (iter = userDataList_.begin(); iter != userDataList_.end(); iter++) {
    ...
    if (memcmp(code, (*iter)->password, SHA256_DIGEST_LENGTH) ==
        0) // every password must be unique
      return false;
  }
...
bool AppConfigManager::changeUserPassword(const char *name,
...
void AppConfigManager::saveConfig() {
  ...
  if (userDataList_.size()) {
    ...
    for (size_t i = 0; i < userDataList_.size(); i++) {
      ...
      encodeBuf =
          ::encodeBase64(userDataList_[i]->password, SHA256_DIGEST_LENGTH);
      if (encodeBuf) {
