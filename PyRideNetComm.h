...
protected:
...
#ifdef PYRIDE_REMOTE_CLIENT
  virtual void
  onRobotCreated(const char cID, const int ipAddr, const RobotInfo *rinfo,
                 const VideoSettings *vsettings, const AudioSettings *asettings,
                 const unsigned char *optLabel, const int optLabelLength) = 0;
  virtual void onRobotDestroyed(const char cID) = 0;
  virtual void onRobotTelemetryData(const char cID, const RobotPose *pose,
                                    const FieldObject *objects = NULL,
  ...
  virtual void onVideoSwitchChange(const char cID,
                                   const VideoSettings *vsettings) = 0;
  virtual void
  onExtendedCommandResponse(const char cID, const PyRideExtendedCommand command,
  ...
  virtual bool executeRemoteCommand(const unsigned char *commandData,
                                    const int dataLength, int &retVal) = 0;
  virtual void cancelCurrentOperation() = 0;
  virtual bool onUserLogOn(const unsigned char *authCode, SOCKET_T fd,
  ...
  virtual int onExclusiveCtrlRequest(SOCKET_T fd) {
    (void)fd;
    return 0;
  }
...
public:
  ...
  void takeCameraSnapshot(const VideoDeviceDataHandler *dataHandler,
...
private:
...
#ifdef PYRIDE_REMOTE_CLIENT
  void processTelemetryData(ClientItem *client,
                            const unsigned char *commandData, int dataLen);
  void processOperationalData(ClientItem *client,
                              const unsigned char *commandData, int dataLen);
  ...
  void clientDataSend(const int command, const int subcommand = 0,
                      const unsigned char *optionalData = NULL,
                      const int optionalDataLen = 0, ClientItem *client = NULL,
                      bool useUDP = false, bool forceAll = false);
