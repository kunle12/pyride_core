...
RTPDataReceiver::RTPDataReceiver()
    : streamSession_(NULL), receiveTimestamp_(0), lastSeqNum_(0),
      dataBuffer_(NULL), dataBufferSize_(0), existDataSize_(0) {}
...
void RTPDataReceiver::init(int port, bool isVideoStream) {
  ...
  if (isVideoStream) {
    ((RTPSession *)streamSession_)
        ->setPayloadFormat(DynamicPayloadFormat(VideoStreamID, 90000));
...
int RTPDataReceiver::grabData(unsigned char **dataBuffer,
...
void RTPDataReceiver::setStreamSource(const char *host, short controlPort,
                                      short dataPort) {
  ...
  if (!hostInfo) { // check for IP
    ...
    if (inet_pton(AF_INET, host, &hostIP) != 1) {
      printf(
          "RTPDataReceiver::setStreamSource: invalid hostname or address %s\n",
          host);
      return;
    }
#endif
    ...
    if (hostInfo == NULL) {
      printf(
          "RTPDataReceiver::setStreamSource: unable to get host info for %s\n",
          host);
      return;
    }
  }
  ...
  // DEBUG_MSG( "got control/data port %d/%d\n", ntohs( controlPort ), ntohs(
  // dataPort ) );
}

void RTPDataReceiver::firewallPunching() {
  // DEBUG_MSG( "Punching time (%d/%d)\n", ntohs(cSourceAddr_.sin_port),
  // ntohs(dSourceAddr_.sin_port) );
  if (sendto(dataSocket_, punchData, 8, 0, (struct sockaddr *)&dSourceAddr_,
             sizeof(dSourceAddr_)) == -1) {
    printf("failed to punch reason %s!\n", strerror(errno));
  }
  sendto(controlSocket_, punchData, 8, 0, (struct sockaddr *)&cSourceAddr_,
         sizeof(cSourceAddr_));
}
