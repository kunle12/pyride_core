...
public:
  ...
  using DeviceController::start;
  using DeviceController::stop;

  virtual bool start(struct sockaddr_in &cAddr,
                     unsigned short cDataPort = PYRIDE_VIDEO_STREAM_BASE_PORT +
                                                2);
  virtual bool stop(struct sockaddr_in &cAddr,
                    unsigned short cDataPort = PYRIDE_VIDEO_STREAM_BASE_PORT +
                                               2);
...
public:
  ...
  virtual void onSnapshotImage(const string &imageName) { (void)imageName; }
};
...
public:
  ...
  virtual bool setCameraParameter(int pid, int value) {
    (void)pid;
    (void)value;
    return true;
  }
...
protected:
  ...
  void processAndSendImageData(const unsigned char *imageData,
                               const int imageDataSize, ImageFormat format);
  void saveToJPEG(const unsigned char *imageData, const int imageDataSize,
                  ImageFormat format);
...
private:
  ...
  bool YUV422ToRGB24(unsigned char *dstData, int dstDataSize,
                     const unsigned char *srcData, const int srcDataSize);
  bool YUV422To24(unsigned char *dstData, int dstDataSize,
                  const unsigned char *srcData, const int srcDataSize);
  bool RGBAToRGB24(unsigned char *dstData, int dstDataSize,
                   const unsigned char *srcData, const int srcDataSize);
  bool BGRAToRGB24(unsigned char *dstData, int dstDataSize,
                   const unsigned char *srcData, const int srcDataSize);
