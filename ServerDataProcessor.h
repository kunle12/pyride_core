...
protected:
  ...
  virtual bool onUserLogOn(const std::string &name) {
    (void)name;
    return false;
  }
  virtual void onUserLogOff(const std::string &name) { (void)name; }

  virtual int onExclusiveCtrlRequest(const std::string &name) {
    (void)name;
    return 0;
  }
...
public:
  ...
  bool dispatchVideoDataTo(int vidObjID, struct sockaddr_in &cAddr, short port,
                           bool todispath);
  bool dispatchAudioDataTo(struct sockaddr_in &cAddr, short port,
                           bool todispath);
  bool setCameraParameter(int vidObjID, int id_idx, int value);

  void updateOperationalStatus(RobotOperationalState status,
                               const char *optionalData = NULL,
  ...
  void setDefaultRobotInfo(const RobotType rtype, const RobotPose &pose,
                           const RobotCapability &capabilities) {
...
private:
  typedef std::vector<PyRideExtendedCommandHandler *>
      PyRideExtendedCommandHandlerList;
  ...
  // RobotDataHandler
  bool executeRemoteCommand(const unsigned char *commandData,
                            const int dataLength, int &retVal);
  void cancelCurrentOperation();
  bool onUserLogOn(const unsigned char *authCode, SOCKET_T fd,
